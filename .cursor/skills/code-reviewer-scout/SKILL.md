...
```bash
curl -s -X POST "http://127.0.0.1:8741/v1/spaces/scout/search" \
...
```bash
python skills/code-reviewer-scout/scripts/review_api.py search scout \
...
```bash
curl -s "http://127.0.0.1:8741/v1/spaces/scout/node/NODE_ID_FROM_HIT"
```

```bash
python skills/code-reviewer-scout/scripts/review_api.py node scout NODE_ID
```
