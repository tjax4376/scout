...
Agent skill template installed by `scout <space> setup` alongside the `add-memory` skill. Agents read `SKILL.md` to call Scout REST API.
...
2. `scout.skill.install.install_skill()` copies `skills/search_scout/` and `skills/add_memory/` to agent paths
